...
ON resource_uploads(multipart_upload_id, upload_status)
...
CREATE INDEX idx_etag_data_gin ON resource_uploads USING gin(etag_data);
...
BEGIN
    ...
    SELECT etag_data INTO v_etag_data
    FROM resource_uploads
    ...
    UPDATE resource_uploads
    SET 
        etag_data = jsonb_set(v_etag_data, '{parts}', v_parts),
...
/*
-- Find all pending uploads for a specific resource
SELECT * FROM resource_uploads 
WHERE resource_type = 'achievement' 
  AND resource_id = '123' 
  AND upload_status = 'pending';

-- Get multipart upload progress
SELECT 
    id,
    multipart_upload_id,
    uploaded_parts || '/' || total_parts AS progress,
    ROUND((uploaded_parts::NUMERIC / total_parts) * 100, 2) || '%' AS percentage
FROM resource_uploads
...
-- MIGRATION NOTES
-- ==================================================================================
-- 1. This schema requires PostgreSQL 14+ for gen_random_uuid()
-- 2. For older versions, use: CREATE EXTENSION IF NOT EXISTS "uuid-ossp"; and uuid_generate_v4()
-- 3. Consider partitioning this table by create_time for very high volume (>10M records)
-- 4. Monitor index usage with pg_stat_user_indexes and adjust as needed
-- 5. Run VACUUM ANALYZE resource_uploads regularly for optimal performance