...
func (h *ProviderHandler) ListProviders(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(
			dto.ErrorResponse("PROVIDERS_ERROR", "Failed to retrieve providers", err.Error()),
		)
	}

	return c.JSON(dto.SuccessResponse(providers))
}
...
func (h *ProviderHandler) GetProvider(c *fiber.Ctx) error {
	...
	if name == "" {
		return c.Status(fiber.StatusBadRequest).JSON(
			dto.ErrorResponse("INVALID_PARAMETER", "Provider name is required", ""),
		)
	}
	...
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(
			dto.ErrorResponse("PROVIDER_NOT_FOUND", "Provider not found", err.Error()),
		)
	}

	return c.JSON(dto.SuccessResponse(provider))
}
